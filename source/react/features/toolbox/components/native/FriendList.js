...
class FriendList extends PureComponent<Props, State> {
    ...
    componentDidMount() {
      ...
      try {
        ...
        this.api_registered_users = this.api_endpoint + 'api/registered_users_vndirect'
        this.api_address_users = this.api_endpoint + 'api/address_book_vndirect'
    ...
    fetchRegisteredUsers(userid) {
      ...
      if (this.token) {
        ...
        const api_address_users = this.api_address_users
        const max_size = 300
        const offset = 0
        const params = {
          offset: offset,
        ...
        .then((json) => {
          if (json && json.items && json.items.constructor === Array && json.items.length > 0) {
            ...
            fetch(api_address_users + '?' + tempBody, {
              method: 'GET',
              headers: { Accept: 'application/json', 'TC-Token': this.token, Connection: 'Keep-Alive', 'Accept-Encoding': 'gzip' }
            }).then((response1) => {
              if (response1 && [404, 401].indexOf(response1.status) !== -1) {
                return {}
              } else {
                try { return response1.json()
                } catch(e1) { return {} }
              }
            })
            .then((json1) => {
              if (json1 && json1.constructor === Array && json1.length > 0) {
                let objContact = {}
                for (let contact of json1) {
                  objContact[contact.phone] = contact.name
                }
                
                arrUserTmp.forEach(userinfo => {
                  if (userinfo && !userinfo.full_name) {
                    userinfo.full_name = objContact[userinfo.phone]
                  }
                })
    ...
    handleSearch(text) {
      let arrUser = this.state.arrUserOrigin || []
      let arrData = []
